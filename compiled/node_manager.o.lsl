...
key nodeIDToKey(string node_id)
{
    ...
    return uncompressKey(llList2String(gNodes, idx + 1));
}
...
string nodeKeyToID(key id)
{
    ...
    if (idx == -1)
        return "";
    return llList2String(gNodes, idx - 1);
}
...
dumpNodeDetails()
{
    ...
    for (i = 0; i < len; i += 2)
    {
        llOwnerSay(llList2String(gNodes, i + 0) + ": " + (string)uncompressKey(llList2String(gNodes, i + 1)));
    }
    len = llGetListLength(gPendingNodes);
    llOwnerSay("gPendingNodes:");
    for (i = 0; i < len; i += 2)
    {
        llOwnerSay(llList2String(gPendingNodes, i + 0) + ": " + llList2String(gPendingNodes, i + 1));
    }
    len = llGetListLength(gNodeRelations);
    llOwnerSay("NODE RELATIONS:");
    for (i = 0; i < len; ++i)
...
untrackNode(string node_id, integer kill_relations)
{
    ...
    if (kill_relations)
    {
        ...
        for (i = len - 1; i >= 0; --i)
        {
            ...
            {
                integer new_rel = new_src_idx << 16 | new_dst_idx;
                gNodeRelations = llListReplaceList(gNodeRelations, [new_rel], i, i);
            }
        }
    }
    gNodes = llDeleteSubList(gNodes, node_idx, node_idx + 2 - 1);
}
...
pruneNodes()
{
    ...
    for (i = len - 2; i >= 0; i -= 2)
    {
        key node_key = uncompressKey(llList2String(gNodes, i + 1));
        if (llGetObjectMass(node_key) == ((float)0))
        {
            untrackNode(llList2String(gNodes, i + 0), 1);
        }
    }
}
...
saveNodes()
{
    ...
    for (i = 0; i < len; i += 2)
    {
        ...
        key node_key = uncompressKey(llList2String(gNodes, i + 1));
        vector node_pos = llList2Vector(llGetObjectDetails(node_key, [3]), 0);
...
addRelation(string src, string dst)
{
    list rel = [llListFindList(gNodes, [src]) << 16 | llListFindList(gNodes, [dst])];
    if (llListFindList(gNodeRelations, rel) == -1)
    {
        gPendingRelations += rel;
        ++gNeededArrows;
...
handleNodeDuplicated(key existing_key, key node_key, string node_id)
{
    ...
    integer node_idx = llListFindList(gNodes, [node_id]);
    integer node_key_idx = node_idx + 1;
    gNodes = llListReplaceList(gNodes, [compressKey(node_key)], node_key_idx, node_key_idx);
...
default
{
    ...
    {
        restoreLogicalID(1);
        gMenuChannel = -21461421 ^ llHash(llGetObjectDesc());
        gConnectionMenuChannel = -21461422 ^ llHash(llGetObjectDesc());
        llListen(((integer)-21461420), "", "", "");
        llListen(((integer)-21461423), "", "", "");
        llListen(gMenuChannel, "", "", "");
        llListen(gConnectionMenuChannel, "", "", "");
        llSetColor(<1, 0, 1>, ((integer)-1));
        clearAll();
        llSetTimerEvent(1);
    }
    ...
    listen(integer channel, string name, key id, string msg)
    {
        ...
        else if (channel == ((integer)-21461423))
        {
            integer num;
            if (cmd == "find_path")
                num = 1000;
    ...
    link_message(integer sender_num, integer num, string str, key id)
    {
        ...
        {
            gPendingRelations += [llListFindList(gPendingNodes, [str]) << 16 | llListFindList(gPendingNodes, [(string)id])];
            ++gNeededArrows;
        }
        else if (num == 901)
        {
    ...
    object_rez(key id)
    {
        ...
        string name = (string)llGetObjectDetails(id, [1]);
        if (name == "node")
        {
            if (gPendingNodes == [])
            {
            ...
            gPendingNodes = llDeleteSubList(gPendingNodes, 0, 2 - 1);
            trackNode(node_id, id);
        }
        else if (name == "arrow")
        {
    ...
    touch_start(integer touch_num)
    {
        if (llDetectedKey(0) != llGetOwner())
            return;
        llDialog(llDetectedKey(0), "Node Management", 
            [ "dump"
            , "save"
            , "restore"
            , "rest. norel"
